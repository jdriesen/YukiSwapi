  ...
  <q-card class="overflow-hidden shadow-3">
    <q-card-section class="q-pa-md">
      ...
      <q-tab-panels v-model="tab" animated transition-prev="slide-right" transition-next="slide-left" class="bg-transparent">
        <!-- Films Tab Panel -->
        <q-tab-panel name="films" v-if="vehicle.films.length > 0" class="q-pa-lg">
          <FilmsTabPanel :resource="vehicle" :film-columns="filmColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>

        <!-- Pilots Tab Panel -->
        <q-tab-panel name="people" v-if="vehicle.pilots.length > 0" class="q-pa-lg">
          <PeopleTabPanel :resource="{ people: vehicle.pilots }" :people-columns="peopleColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>
        ...
        <q-tab-panel v-if="availableTabs.length === 0" name="no-data" class="q-pa-xl">
          <div class="text-center">
            ...
            <p class="text-body2 text-grey-6 q-mt-md q-mb-none">This vehicle has no associated films or pilots data available.</p>
          </div>
        </q-tab-panel>
      </q-tab-panels>
    </q-card-section>
...
/**
 * VehicleTabs Component
 * Displays vehicle-related information in a tabbed interface
 * Uses dynamic tabs based on available data and global resource configuration
 */

interface Props {
  /** The vehicle object containing all related data arrays */
  vehicle: Vehicle
...
// Create reactive basic columns for VehicleTabs context
...
// Create dynamic tabs based on vehicle data and resource configuration
const availableTabs = computed<TabConfig[]>(() => {
  const tabs: TabConfig[] = []

  // Map vehicle properties to resource types with their data
  const vehicleDataMap = [
    { resourceType: 'films' as ResourceType, data: props.vehicle.films },
    { resourceType: 'people' as ResourceType, data: props.vehicle.pilots }, // Map pilots to people
  ]

  // Only create tabs for data that exists
  vehicleDataMap.forEach(({ resourceType, data }) => {
    if (data.length > 0) {
      tabs.push({
        name: resourceType,
        resourceType,
