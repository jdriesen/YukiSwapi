  ...
  <q-card class="overflow-hidden shadow-3">
    <q-card-section class="q-pa-none">
      ...
      <q-tab-panels v-model="tab" animated transition-prev="slide-right" transition-next="slide-left" class="bg-transparent">
        <!-- Films Tab Panel -->
        <q-tab-panel name="films" v-if="species.films.length > 0" class="q-pa-lg">
          <FilmsTabPanel :resource="species" :film-columns="filmColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>

        <!-- People Tab Panel -->
        <q-tab-panel name="people" v-if="species.people.length > 0" class="q-pa-lg">
          <PeopleTabPanel :resource="{ people: species.people }" :people-columns="peopleColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>
        ...
        <q-tab-panel v-if="availableTabs.length === 0" name="no-data" class="q-pa-xl">
          <div class="text-center">
            ...
            <p class="text-body2 text-grey-6 q-mt-md q-mb-none">This species has no associated films or people data available.</p>
          </div>
        </q-tab-panel>
      </q-tab-panels>
    </q-card-section>
...
import { getResourceIcon, getResourceColor, getResourceLabelPlural, type ResourceType } from '~/config/resource-icons'
import type { Species } from '~/types'

/**
 * SpeciesTabs Component
 * Displays species-related information in a tabbed interface
 * Uses dynamic tabs based on available data and global resource configuration
 */

interface Props {
  /** The species object containing all related data arrays */
  species: Species
...
// Create reactive basic columns for SpeciesTabs context
...
// Create dynamic tabs based on species data and resource configuration
const availableTabs = computed<TabConfig[]>(() => {
  const tabs: TabConfig[] = []

  // Map species properties to resource types with their data
  const speciesDataMap = [
    { resourceType: 'films' as ResourceType, data: props.species.films },
    { resourceType: 'people' as ResourceType, data: props.species.people },
  ]

  // Only create tabs for data that exists
  speciesDataMap.forEach(({ resourceType, data }) => {
    if (data.length > 0) {
      tabs.push({
        name: resourceType,
        resourceType,
        ...
        label: getResourceLabelPlural(resourceType),
        count: data.length,
      })
    }
  })
