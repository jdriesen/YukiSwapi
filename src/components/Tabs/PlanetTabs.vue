  ...
  <q-card class="overflow-hidden shadow-3">
    <q-card-section class="q-pa-none">
      ...
      <q-tab-panels v-model="tab" animated transition-prev="slide-right" transition-next="slide-left" class="bg-transparent">
        <!-- Films Tab Panel -->
        <q-tab-panel name="films" v-if="planet.films.length > 0" class="q-pa-lg">
          <FilmsTabPanel :resource="planet" :film-columns="filmColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>

        <!-- Residents Tab Panel -->
        <q-tab-panel name="people" v-if="planet.residents.length > 0" class="q-pa-lg">
          <PeopleTabPanel :resource="{ people: planet.residents }" :people-columns="peopleColumns" :get-id-from-url="getIdFromUrl" />
        </q-tab-panel>
        ...
        <q-tab-panel v-if="availableTabs.length === 0" name="no-data" class="q-pa-xl">
          <div class="text-center">
            ...
            <p class="text-body2 text-grey-6 q-mt-md q-mb-none">This planet has no associated films or residents data available.</p>
          </div>
        </q-tab-panel>
      </q-tab-panels>
    </q-card-section>
...
// Films - Now using shared component
import { createBasicFilmColumns } from '~/config/films/columns'

// People - Now using shared component
import { createBasicPeopleColumns } from '~/config/people/columns'

/**
 * PlanetTabs Component
 * Displays planet-related information in a tabbed interface
 * Uses dynamic tabs based on available data and global resource configuration
 * FilmsTable uses explicit column configuration for different contexts
 */

interface Props {
  /** The planet object containing all related data arrays */
  planet: Planet
...
// Create reactive basic people columns for PlanetTabs context
const peopleColumns = computed(() => createBasicPeopleColumns(t))

// Tab state management
const tab = ref<string>('')

// Create dynamic tabs based on planet data and resource configuration
const availableTabs = computed<TabConfig[]>(() => {
  const tabs: TabConfig[] = []

  // Map planet properties to resource types with their data
  const planetDataMap = [
    { resourceType: 'films' as ResourceType, data: props.planet.films },
    { resourceType: 'people' as ResourceType, data: props.planet.residents }, // Map residents to people
  ]

  // Only create tabs for data that exists
  planetDataMap.forEach(({ resourceType, data }) => {
    if (data.length > 0) {
      tabs.push({
        name: resourceType,
        resourceType,
